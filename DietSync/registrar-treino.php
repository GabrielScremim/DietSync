...
if (isset($_GET['id_treino_editar'])) {
    ...
    if ($dados_treino) {
        ...
        $dia_semana = $dados_treino['dia_treino'];
    } else {
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $data = $_POST["data"];
    $tipo = $_POST["tipo"];
    $exercicios = json_encode(explode(',', $_POST['exercicios'])); // Corrigido para json_encode
    ...
    $dia_semana = $_POST["dia_treino"];

    // Verifica se está editando um treino existente
    if (isset($_GET['id_treino_editar'])) {
        $id_treino = $_GET['id_treino_editar'];
        $treino->EditarTreino($id_treino, $data, $tipo, $exercicios, $repeticoes, $series, $objetivo, $duracao, $frequencia, $nome_treino, $dia_semana);
    } else {
        $treino->AdicionarTreino($data, $tipo, $exercicios, $repeticoes, $series, $objetivo, $duracao, $frequencia, $nome_treino, $user_id, $dia_semana);
    }
}
?>
...
<div class="container" id="main">
    ...
    <form method="post">
        ...
        <div class="row my-5">
            ...
            <div class="col-lg-2 col-sm-12">
                <label for="dia_treino">Dia da Treino</label>
                <select name="dia_treino" id="dia_treino" class="form-control">
                    <option value="" disabled selected>Selecione</option>
                    <option value="1" <?php echo (isset($dia_semana) && $dia_semana == '1') ? 'selected' : ''; ?>>Segunda-feira</option>
                    <option value="2" <?php echo (isset($dia_semana) && $dia_semana == '2') ? 'selected' : ''; ?>>Terça-feira</option>
                    <option value="3" <?php echo (isset($dia_semana) && $dia_semana == '3') ? 'selected' : ''; ?>>Quarta-feira</option>
                    <option value="4" <?php echo (isset($dia_semana) && $dia_semana == '4') ? 'selected' : ''; ?>>Quinta-feira</option>
                    <option value="5" <?php echo (isset($dia_semana) && $dia_semana == '5') ? 'selected' : ''; ?>>Sexta-feira</option>
                    <option value="6" <?php echo (isset($dia_semana) && $dia_semana == '6') ? 'selected' : ''; ?>>Sábado</option>
                    <option value="7" <?php echo (isset($dia_semana) && $dia_semana == '7') ? 'selected' : ''; ?>>Domingo</option>
                </select>
            </div>
